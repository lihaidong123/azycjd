...
<div class="divInit">
	...
	<form id="AnzhiNovelForm" action="anzhinovel/novelTitleUpdateCheck.do" method="post">
		<div class="searchItemsCssss">
			...
			<div style="clear: both; width:80%;">
				<input class="searchBtnCss" type="button" value="查询" onclick="direcNum('anzhinovel/novelTitleUpdateCheck.do','AnzhiNovelForm')"/>
			</div>
		</div>
		<div class="tableCss">
			<table width="100%" border="0">
				...
				<c:forEach items="${anzhinovelList.list}" var="s" varStatus="ind">
					<tr>
						<td>${ind.index+1 }</td>
						<td><a href="<%=request.getContextPath()%>/novel/${s.novelId }.jsp" target="_blank">${s.novelTitle }</a>/<font style="color:red;">${s.vote }</font></td>
						...
						<td>
							<c:if test="${s.state eq 0 }">
								<a id='updateanzhinovel${s.id}' class='updateBtnCss' href='javascript:;' onclick="loadUpdatePageData('anzhinovel/findAnzhiNovelById.do?novelId=${s.novelId}&&tableId=${s.id }&&pageName=admin/novelTitleUpdateCheckAction',this,'updateanzhinovel')">[处理]</a>
							</c:if>
						</td>
					</tr>
					<!-- 加载处理结果 -->
			...
			<jsp:include page="../next_page.jsp" flush="false">
				<jsp:param value="anzhinovel/novelTitleUpdateCheck.do" name="url"/>
