...
<div class="divInit">
	<div class="divInitOne">
		<div class="divInitTwo">
			章节删除审核管理&nbsp;&nbsp;&nbsp;[<a href="javascript:;" onclick="allLoadPage('<%=request.getContextPath()%>/anzhihandle/AnzhiHandleManager.do?handleType=11')">章节删除审核记录查询</a>]
		</div>
	</div>
	<form id="AnzhiChapterForm" action="anzhichapter/findAnzhiChapterAll.do" method="post">
		<div class="searchItemsCssss">
			...
			<div>
				<input class="searchBtnCss" type="button" value="查询" onclick="direcNum('anzhichapter/delChapterCheck.do','AnzhiChapterForm')"/>
			</div>
		</div>
		<div class="tableCss">
					...
					<tr>
						...
						<td>
							<c:if test="${s.state eq 0 }">
								<a id='updateanzhichapter${s.id}' class='updateBtnCss' href='javascript:;' onclick="loadUpdatePageData('anzhichapter/findAnzhiChapterById.do?anzhichapterId=${s.novelId}&&tableId=${s.id }&&pageName=admin/delChapterCheckAction',this,'updateanzhichapter')">[处理]</a>
							</c:if>
						</td>
					</tr>
					<!-- 加载处理结果 -->
			...
			<jsp:include page="../next_page.jsp" flush="false">
				<jsp:param value="anzhichapter/delChapterCheck.do" name="url"/>
