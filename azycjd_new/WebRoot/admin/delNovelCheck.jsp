...
<div class="divInit">
	<div class="divInitOne">
		<div class="divInitTwo">
			作家删文审核&nbsp;&nbsp;&nbsp;[<a href="javascript:;" onclick="allLoadPage('<%=request.getContextPath()%>/anzhihandle/AnzhiHandleManager.do?handleType=10')">作家删文审核记录查询</a>]
		</div>
	</div>
	<form id="AnzhiNovelForm" action="anzhinovel/delNovelCheck.do" method="post">
		<div class="searchItemsCssss">
			...
			<div style="clear: both; width:80%;">
				<input class="searchBtnCss" type="button" value="查询" onclick="direcNum('anzhinovel/delNovelCheck.do','AnzhiNovelForm')"/>
			</div>
		</div>
		<div class="tableCss">
					...
					<tr>
						...
						<td>
							<c:if test="${s.state eq 0 }">
								<a id='updateanzhinovel${s.id}' class='updateBtnCss' href='javascript:;' onclick="loadUpdatePageData('anzhinovel/findAnzhiNovelById.do?novelId=${s.novelId}&&tableId=${s.id }&&pageName=admin/delNovelCheckAction',this,'updateanzhinovel')">[处理]</a>
							</c:if>
						</td>
					</tr>
					<!-- 加载处理结果 -->
			...
			<jsp:include page="../next_page.jsp" flush="false">
				<jsp:param value="anzhinovel/delNovelCheck.do" name="url"/>
