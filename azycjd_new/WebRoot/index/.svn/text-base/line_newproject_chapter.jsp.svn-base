...
<form class="formCss" id="day_chapter_form" action="<%=request.getContextPath() %>/anzhinovel/addAnzhiNovelNew.do?step=two" method="post">
	<div>
		...
		<input type="hidden" id="novelId" name="novelId" value="0">
		<input type="hidden" id="fenJuanId" name="fenJuanId" value="1">
		<input type="hidden" id="state" name="state" value="0">
		<div class="divItemsCss">所在分卷:
			<input type="text" style="width:370px;" class="selectTxtCss" id="fenJuanIdNewInput" name="fenJuanIdNewInput" value="1">
		</div>
		<div class="divItemsCss">章节名称:
			<input type="text" class="txtCss" id="chapterTitle" name="chapterTitle" value="${pojo.chapterTitle }">
			<!-- <input type="button" value="显示时间" class="saveBtnCss" onclick="$('#dabusijian').slideToggle()"> -->
		</div>
